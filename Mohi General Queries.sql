...
/* write query to display all employees information */
SELECT *
FROM employees;

/* write query to display the first name, last name, job ID, and salary only for employees those their first name contain s*/
SELECT first_name,last_name,job_id,salary
FROM employees
WHERE first_name LIKE 's%';

/*write query to display the employees with the highest salary*/
SELECT *
FROM employees 
WHERE salary = (SELECT MAX(salary) FROM employees);

/*write query to display the salary in from high to low*/
SELECT *
FROM employees
ORDER BY salary DESC;

/*write query to display the information of the employee who ranked as second one in his/her income*/
...
SELECT *
FROM RankedSalaries
WHERE SalaryRank=2 OR SalaryRank=3;


/*write query to display count of employees and their manager*/
SELECT COUNT(employee_id) AS emp_count, manager_id
FROM employees
GROUP BY manager_id;

/*write query to group the employees based on their hiring date*/
SELECT COUNT(employee_id)AS emp_count, hire_date
FROM employees
GROUP BY hire_date;

/* write query to arrange the employee from largest to lowest based on their salaries*/

SELECT employee_id,salary
FROM employees
ORDER BY salary ASC;

/*write query to categorize the employees into 5 groups based on their salaries*/
SELECT 
SUM(CASE WHEN salary BETWEEN 2200 AND 6000 THEN 1 ELSE 0 END) AS Range_2200_to_6000,
SUM(CASE WHEN salary BETWEEN 6500 AND 11000 THEN 1 ELSE 0 END) AS Range_6500_to_11000,
SUM(CASE WHEN salary BETWEEN 12000 AND 24000 THEN 1 ELSE 0 END) AS Range_12000_to_24000
FROM employees;

SELECT employee_id,
SUM(CASE WHEN salary BETWEEN 2200 AND 6000 THEN 1 ELSE 0 END) AS Range1,
SUM(CASE WHEN salary BETWEEN 6500 AND 11000 THEN 1 ELSE 0 END) AS Range2,
SUM (CASE WHEN salary BETWEEN 12000 AND 24000 THEN 1 ELSE 0 END) AS Range3
FROM employees
GROUP BY employee_id;

/*write query to display all employees who's their first name contain an*/
SELECT first_name
FROM employees
WHERE first_name LIKE '%an%';

/* write query to display the employees who hired in Aug 1994 */
SELECT first_name,hire_date
FROM employees
WHERE hire_date BETWEEN '1994-08-01' AND '1994-08-30';

/* write query to show employees those their salary greater than overall average salary*/
SELECT employee_id,salary
FROM employees
WHERE salary > (select AVG(salary) FROM employees);

/*write query to display the high earned employees and their working department*/ 
SELECT MAX(salary) AS Max_salary,department_id
FROM employees
GROUP BY department_id;

/* write query to show the first 5 employees in the records*/
SELECT TOP 5 employee_id,salary
FROM employees
ORDER BY salary;

/*write query to show the employees who hired between 1980 and 1989*/

SELECT employee_id,hire_date
FROM employees
WHERE year(hire_date) BETWEEN 1980 AND 1989;

/*write query to reverse the first name of the employees*/
SELECT first_name, REVERSE(first_name)
FROM employees;

/*write query to show employees who hired after 15th of the month */
SELECT employee_id,hire_date
FROM employees 
